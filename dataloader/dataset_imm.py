...
def load_imm_data(data_path, current_file):
    A = current_file
    data_path = os.getcwd() + data_path
    filename = data_path + 'frames_finalpass/' + A[0: len(A) - 1]
    left_bw = Image.open(filename)
    left_bw = np.asarray(left_bw)
    size = np.shape(left_bw)
    height = size[0]
    width = size[1]
    left = np.zeros([height, width, 3], 'int')
    left[:, :, 0] = left_bw
    left[:, :, 1] = left_bw
    left[:, :, 2] = left_bw
    filename = data_path + 'frames_finalpass/' + A[0: len(A) - 1].replace("left", "right").replace("LEFT", "RIGHT")
    right_bw = Image.open(filename)
    right_bw = np.asarray(right_bw)
    right = np.zeros([height, width, 3], 'int')
    right[:, :, 0] = right_bw
    right[:, :, 1] = right_bw
    right[:, :, 2] = right_bw
    filename = data_path + 'disparity/' + A[0: len(A) - 1].replace("RGB", "AGL")
    disp_left = Image.open(filename)
    filename = data_path + 'disparity/' + A[0: len(A) - 1].replace("left", "right").replace("LEFT", "RIGHT").replace("RGB", "AGL")
    disp_right = Image.open(filename)
    temp_data = np.zeros([8, height, width], 'float32')
    disp_left = np.asarray(disp_left)
    disp_right = np.asarray(disp_right)
    r = left[:, :, 0]
    g = left[:, :, 1]
    b = left[:, :, 2]
...
class DatasetFromList(data.Dataset): 
    ...
    def __getitem__(self, index):
        ...
        if self.kitti: #load kitti dataset
            ...
            temp_data = load_imm_data(self.data_path, self.file_list[index])
        if self.training:
