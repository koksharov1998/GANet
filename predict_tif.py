...
from dataloader.data_tif import get_test_set
import numpy as np
import tifffile
...
def load_data(leftname, rightname):
    # print(os.getcwd())
    left = tifffile.imread(os.getcwd() + leftname)
    right = tifffile.imread(os.getcwd() + rightname)
    size = np.shape(left)
    height = size[0]
    width = size[1]
...
def test(leftname, rightname, savename):
  #  count=0
    ...
    tifffile.imsave(savename, (temp * 256), compress=6)
